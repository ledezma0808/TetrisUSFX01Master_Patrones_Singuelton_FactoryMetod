...
void APiece::SpawnBlocks()
{
    ...
    ABlock* B = nullptr;
   // int Tipo = FMath::FRandRange(0, 1);
   // FString _TipoBloque = ABlockArena
    FString _TipoBloque = ABlock::getBloque(); 

    if (_TipoBloque == "ARENA") {
        for (auto&& YZ : YZs)
        {
            FRotator Rotation(0.0, 0.0, 0.0);
            B = GetWorld()->SpawnActor<ABlockArena>(this->GetActorLocation(), Rotation);
    ...
    if (_TipoBloque == "COLORIDO") {
        for (auto&& YZ : YZs)
        {
            FRotator Rotation(0.0, 0.0, 0.0);
            B = GetWorld()->SpawnActor<ABlockColorido>(this->GetActorLocation(), Rotation);
            B->BlockMesh->SetMaterial(1, Colors[Index]);
    ...
    if (_TipoBloque == "GRASS") {

        for (auto&& YZ : YZs)
        {
            FRotator Rotation(0.0, 0.0, 0.0);
            B = GetWorld()->SpawnActor<ABlockGrass>(this->GetActorLocation(), Rotation);
            B->BlockMesh->SetMaterial(1, Colors[Index]);
     ...
     if (_TipoBloque == "MARFIL") {
         for (auto&& YZ : YZs)
         {
             FRotator Rotation(0.0, 0.0, 0.0);
             B = GetWorld()->SpawnActor<ABlockMarfil>(this->GetActorLocation(), Rotation);
             B->BlockMesh->SetMaterial(1, Colors[Index]);
             Blocks.Add(B);
             B->AttachToActor(this, FAttachmentTransformRules::KeepRelativeTransform);
             B->SetActorRelativeLocation(FVector(0.0, YZ.first, YZ.second));
...
void APiece::DrawDebugLines()             //Sirve para dibujar como guías, se usa en tiempo de desarrollo
{
    for (ABlock* B : Blocks)
    {
...
{
    auto RotateVector = [this](FVector OldVector) {       //Esto permite rotar. El código está en forma de una funcion LAMBDA
